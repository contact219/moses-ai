...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are Moses, an advanced AI assistant. "
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "close the assistant, say goodbye, or stop Moses. "
...
class MosesLive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[MOSES] 📤 {name} → {str(result)[:80]}")
        return types.FunctionResponse(
            id=fc.id, name=name,
    ...
    async def _listen_audio(self):
        ...
        def callback(indata, frames, time_info, status):
            with self._speaking_lock:
                moses_speaking = self._is_speaking
            if not moses_speaking and not self.ui.muted:
                data = indata.tobytes()
    ...
    async def _receive_audio(self):
        ...
        try:
                ...
                async for response in self.session.receive():
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if full_out:
                                self.ui.write_log(f"Moses: {full_out}")
                            out_buf = []

                    if response.tool_call:
                        fn_responses = []
                        for fc in response.tool_call.function_calls:
                            print(f"[MOSES] 📞 {fc.name}")
            ...
            print(f"[MOSES] ❌ Recv: {e}")
            traceback.print_exc()
            raise

    async def _play_audio(self):
        print("[MOSES] 🔊 Play started")
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    print("[MOSES] ✅ Connected.")
                    self.ui.set_state("LISTENING")
                    self.ui.write_log("SYS: Moses online.")
            ...
            self.set_speaking(False)
            self.ui.set_state("THINKING")
            print("[MOSES] 🔄 Reconnecting in 3s...")
