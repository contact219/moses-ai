...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".moses_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".moses_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using Moses profile")
                moses_path = str(Path.home() / ".moses_profiles" / "firefox_moses")
                Path(moses_path).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(moses_path, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".moses_profiles" / "safari")
        ...
        moses_profile = str(Path.home() / ".moses_profiles" / self.browser_name)
        Path(moses_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with Moses profile: {moses_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(moses_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with Moses profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "moses_screenshot.png")
